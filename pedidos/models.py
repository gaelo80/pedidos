...
class Pedido(models.Model):
    ESTADO_PEDIDO_CHOICES = [
        ...
        ('APROBADO_ADMIN', 'Aprobado por Administración (Listo Bodega)'), # Estado final para Bodega
    ...
    estado = models.CharField(
        max_length=35, # Ajustar longitud para los nuevos valores más largos
    ...
    porcentaje_descuento = models.DecimalField(
        max_digits=5, decimal_places=2, default=Decimal('0.00'), verbose_name="Descuento Aplicado (%)"
    )
    token_descarga_fotos = models.UUIDField(default=uuid.uuid4, unique=True, editable=False)

    # --- Campos de Trazabilidad Cartera ---
    motivo_cartera = models.TextField(blank=True, null=True, verbose_name="Motivo/Notas Cartera")
    usuario_decision_cartera = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.SET_NULL,
        null=True, blank=True,
        related_name='pedidos_decision_cartera',
        verbose_name="Decisión Tomada por (Cartera)"
    )
    fecha_decision_cartera = models.DateTimeField(null=True, blank=True, verbose_name="Fecha Decisión Cartera")

    # --- Campos de Trazabilidad Administración ---
    motivo_admin = models.TextField(blank=True, null=True, verbose_name="Motivo/Notas Administración")
    usuario_decision_admin = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.SET_NULL,
        null=True, blank=True,
        related_name='pedidos_decision_admin',
        verbose_name="Decisión Tomada por (Admin.)"
    )
    ...
    # ... (tus propiedades como subtotal_base_bruto, total_a_pagar, etc. no cambian) ...
    @property
